...
// ---------------------------------------- UPDATE ATTRIBUTE SETS ---------------------------------------- //
    ...
    if (false === $foundAttributeSet) {
        ...
        if (false === empty($parentIds)) {
            //var_dump($parentIds[0]); exit;
//        $productName = $productRepository->getById($parentIds[0])->getName();

            $parentProduct = $productModel->load($parentIds[0]);

//            $parentProductCollection = $productCollection;
//            $parentProduct = $parentProductCollection->addFieldToFilter('entity_id', $parentIds[0])->getFirstItem();
            ...
            if (false === $foundAttributeSet) {
                ...
                $logger->debug('Not found attribute fot for: ' . $productName);
